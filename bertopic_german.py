...
def read_csv(path):
    ...
    csv = pd.read_csv(path, sep = ";", on_bad_lines='skip')
    documents = csv.iloc[:, 1].tolist()
    print(len(documents))
    return documents[:200] # limit to first 200 documents

documents = read_csv("articles.csv") # german news articles
...
#sentence_model = SentenceTransformer("paraphrase-multilingual-mpnet-base-v2") # important to use a multilingual for german
...
# py -m spacy download de_core_news_trf (or other german models)
#import spacy
#spacy.prefer_gpu()
#sentence_model = spacy.load("de_core_news_trf", exclude=['tagger', 'parser', 'ner', 'attribute_ruler', 'lemmatizer'])
...
#print(list(german_stop_words))
german_stop_words = list(set(list(german_stop_words) + ["der", "die", "das"])) # you can add more stopwords here
...
vect_model = CountVectorizer(
    ...
    stop_words=german_stop_words,  # Remove stop words
...
#vect_model = CountVectorizer(stop_words="english") for english or use english nltk stopwords
...
#            [{'POS': 'NOUN'}], [{'POS': 'ADJ'}]
#]
#representation_model = PartOfSpeech("de_core_news_sm", pos_patterns=pos_patterns) # py -m spacy download de_core_news_sm
...
topic_model = BERTopic(
    # Step 1: Embedding
    language="german", # if this is set, embedding_model will be ignored
